...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Deleting field 'ProductItem.target_page'
        db.delete_column(u'djangocms_product_productitem', 'target_page_id')

        # Removing M2M table for field sites on 'ProductItem'
        db.delete_table(db.shorten_name(u'djangocms_product_productitem_sites'))

        # Adding M2M table for field target_page on 'ProductItem'
        m2m_table_name = db.shorten_name(u'djangocms_product_productitem_target_page')
        ...
        db.create_unique(m2m_table_name, ['productitem_id', 'page_id'])


    def backwards(self, orm):

        # User chose to not deal with backwards NULL issues for 'ProductItem.target_page'
        raise RuntimeError("Cannot reverse this migration. 'ProductItem.target_page' and its values cannot be restored.")
        
        # The following code is provided here to aid in writing a correct migration        # Adding field 'ProductItem.target_page'
        db.add_column(u'djangocms_product_productitem', 'target_page',
                      self.gf('django.db.models.fields.related.ForeignKey')(to=orm['cms.Page']),
                      keep_default=False)
        ...
        # Removing M2M table for field target_page on 'ProductItem'
        db.delete_table(db.shorten_name(u'djangocms_product_productitem_target_page'))
